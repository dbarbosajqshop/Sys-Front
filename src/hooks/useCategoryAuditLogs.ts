...
import { getCategoryAuditLogs } from "@/services/auditLogs"; 
import { IAuditLogCategoryResponse, IFetchCategoryLogsParams } from "@/types/auditLogs";
import { AxiosResponse } from "axios";

export const useCategoryAuditLogs = () => {
  const [logs, setLogs] = useState<IAuditLogCategoryResponse>();
  const [loading, setLoading] = useState(true);
  const navigate = useNavigate();

  const fetchLogs = useCallback(async (params: IFetchCategoryLogsParams = {}) => {
    try {
      setLoading(true);
      const response = await getCategoryAuditLogs(params);
      ...
      if ((response as AxiosResponse).status) {
        ...
        if (errorResponse.status === 404) { setLogs({ data: [], pagination: { total: 0, page: 0, limit: 0, totalPages: 0 } }); toast.error(errorResponse.data?.message || errorResponse.data?.error, { theme: "colored" }); return; }
        if (errorResponse.status !== 200) { toast.error(errorResponse.data?.message || errorResponse.data?.error, { theme: "colored" }); return; }
      }

      setLogs(response as IAuditLogCategoryResponse);
    } catch (error) {
