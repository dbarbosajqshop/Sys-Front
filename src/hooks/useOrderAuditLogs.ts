...
import { getOrderAuditLogs } from "@/services/auditLogs";
import { IAuditLogOrderResponse, IFetchOrderLogsParams } from "@/types/auditLogs";
import { AxiosResponse } from "axios";

export const useOrderAuditLogs = () => {
  const [logs, setLogs] = useState<IAuditLogOrderResponse>({
    data: [],
  ...
  const fetchLogs = useCallback(async (params: IFetchOrderLogsParams = {}) => {
    try {
      setLoading(true);
      const response = await getOrderAuditLogs(params);
      ...
      setLogs(response as IAuditLogOrderResponse);
    } catch (error) {
