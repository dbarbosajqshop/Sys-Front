...
import { DeliverConfirmationModal } from "@/ui/DeliverConfirmationModal";
import { useProfile } from "@/hooks/useProfile";
import { toast } from "react-toastify";

export default function OrderDocks() {
  ...
  const handleDockChange = () => {
    addDockToOrder(orderId, dockCode);
    ...
    <div className="flex flex-col gap-6 bg-neutral-0 p-sm border border-neutral-200 rounded-nano">
      {!loading && (
        <>
          <DockModal
            open={openDockModal}
            setOpen={setOpenDockModal}
            dock={dockCode}
            setDock={setDockCode}
            onConfirm={handleDockChange}
          />
          <DeliverConfirmationModal
            open={openDeliverModal}
            setOpen={setOpenDeliverModal}
            onConfirm={() => {
        ...
        orders && (
          <>
            <Table
              ...
              onDeliver={
                authorize("w_docks_orders") ? () => setOpenDeliverModal(true) : undefined
              }
              onDock={
                authorize("w_docks_orders") ? () => setOpenDockModal(true) : undefined
              }
              onDetail={
                authorize("r_docks_orders")
