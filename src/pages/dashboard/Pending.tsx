...
export default function Pending() {
  ...
  const { authorize, profile } = useProfile(); 
  ...
  const isSeller = profile?.Roles.some(
    (role) => role.name === "seller_online" || role.name === "seller_local"
  );
          ...
          <>
            <Table
              ...
              headers={[
                ...
                ...(isSeller ? [] : [{ label: "Vendedor", key: "SellerId.name" }]),
              ...
              onDetail={
                authorize("w_pending_orders")
                  ? () => navigate(`/dashboard/pending/${orderId}`)
                  : undefined
              }
              detailText="Ajustar venda"
              deleteTitle="Você tem certeza que deseja excluir essa venda?"
              deleteDescription="Ao excluir essa venda não será possível recuperá-la e ela será excluída das suas métricas de venda"
            />
            <Pagination
